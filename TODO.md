...
- [ ] **Notifica email nuove richieste**: Edge Function scritta e distribuita (`supabase/functions/notify-new-quote`) ma non collegata — serve un account Resend, le chiavi `RESEND_API_KEY`/`NOTIFY_EMAIL` nei secrets, e il Database Webhook su Supabase. **Nota**: controllato il 2026-09-01, il webhook non esiste ancora (stessa cosa che mancava per la newsletter, vedi sotto) — posso collegarlo io via SQL appena hai le chiavi Resend, stesso procedimento già fatto per MailerLite.
